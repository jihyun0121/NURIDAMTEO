...
export default function FaqList() {
    ...
    return (
        <div className="nurisodam-list-container">
            ...
            <Faq id={4} active={activeId === 4} onClick={() => handleToggle(4)} question="누리담터 마일리지는 무엇인가요?" anwer="누리담터 회원으로 가입하고 활동하면 부여 기준에 따라 적립되는 활동 장려금입니다. 1만 점 이상 적립 시 누리소담페이로 전환이 가능합니다" />
            <Faq id={5} active={activeId === 5} onClick={() => handleToggle(5)} question="누리담터 마일리지는 어떻게 모으나요?" anwer={"○ 마일리지 적립기준<br>회원가입, 가입추천인 입력, 출석체크, 제안 글을 올리거나 댓글을 달면 마일리지가 적립됩니다.<br><br>○ 마일리지를 누리소담페이로 전환해드립니다.<br>- 신청대상 : 마일리지 1만점 이상 회원 (1만점 신청)<br>- 신청시기 : 매월<br>- 신청방법 : 마일리지 > 누리소담페이 전환 클릭<br>- 지급시기 : 신청일 익월 (※ 예산범위 내 지급)"} />
            ...
            <Faq id={8} active={activeId === 8} onClick={() => handleToggle(8)} question="누리소담돌봄서비스는 어떻게 이용할 수 있나요?" anwer="주소지 동 행정복지센터에 전화 또는 방문하거나, 누리담터 모바일 앱을 통해 신청하면 동 돌봄 플래너에게 연결됩니다" />
        </div>
    );
}
