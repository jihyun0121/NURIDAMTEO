...
export default function PanelForm({ survey }) {
    ...
    useEffect(() => {
        ...
        if (survey?.survey_type === "SELECT" || survey?.survey_type === "SURVEY") {
            setIsSelect(true);
            return;
        }
        ...
        async function fetchSelection() {
            try {
                const res = await SurveyAPI.checkSurveySelection(survey.survey_id, userId);
                setIsSelect(res.data);
            } catch (e) {
                console.log("선정 여부 조회 실패", e);
            }
        }

        fetchSelection();
    }, [survey?.survey_type, survey?.survey_id, userId]);
    ...
    async function init() {
        try {
            ...
            const mySurveyParticipation = myParticipations.find((p) => Number(p.target_id) === Number(survey.survey_id) && ["PANEL", "SURVEY"].includes(String(p.target_type).toUpperCase()));
    ...
    async function submit() {
        ...
        try {
            const participationTargetType = survey?.survey_type === "PANEL" ? "PANEL" : "SURVEY";

            const participationRes = await ParticipationAPI.createParticipaiton({
                user_id: Number(userId),
                target_type: participationTargetType,
    ...
    return (
        <div className="panel-container">
            {!isSubmitted ? (
                !isSelect ? (
                    <div className="panel-content-none">
                        <p>
                            본 패널 조사 대상자 선정 결과, 귀하는 이번 조사에는 아쉽게도 선정되지 않으셨음을 안내드립니다.
                            <br></br>
                            소중한 시간 내어 본 조사에 관심을 가지고 참여해 주신 점에 대해 깊이 감사드립니다.
                            <br></br>
                            귀하께서 제출해 주신 응답은 향후 정책 수립 및 서비스 개선을 위한 참고 자료로 활용될 예정입니다.
                            <br></br>
                            앞으로도 진행될 각종 조사 및 시민 참여 프로그램에 지속적인 관심과 참여를 부탁드리며, 다시 한 번 감사의 말씀을 드립니다.
                        </p>
                    </div>
                ) : (
                    <>
                ...
                <div className="survey-content-text">
                    <div dangerouslySetInnerHTML={{ __html: survey?.result }} />
                </div>
            )}
            <div className="answer-buttons">
                <TextButtonS content="목록으로" onClick={() => navigate(-1)} />
                {isSelect && <TextButtonS content={isSubmitted ? "제출완료" : loading ? "제출 중..." : "제출하기"} type={isSubmitted ? "default" : "hover"} onClick={submit} disabled={loading || isSubmitted} />}
            </div>
        </div>
    );
}
